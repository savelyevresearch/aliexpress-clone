<template>
    <div class="flex justify-start my-2">
        ...
        <NuxtImg class="rounded-md md:w-[150px] w-[90px]" :src="product.url" />
        <div class="overflow-hidden pl-2 w-full">
            <div class="flex items-center justify-between w-full">
                ...
                <button @click="removeFormCart()" class="mx-3 sm:block hidden -mt-0.5 hover:text-red-500">
...
const removeFromCart = () => {
    userStore.cart.forEach((cartProduct, index) => {
        if (cartProduct.id === product.value.id) {
            userStore.cart.splice(index, 1);
        }
    });
};

watch(() => isSelected.value, (newValue, oldValue) => {
    emit("selectedRadio", { id: product.value.id, val: newValue })
});
</script>